...
def visualize_evaluations(
    ...
    clusterers = sorted(list(set([i.split(param_delim, 1)[0] for i in evaluations_df.columns])))
    evaluations_df = zscore(evaluations_df)
    width = 0.18*(len(evaluations_df.columns)+3+(0.01*(len(clusterers)-1)))
    ...
    for i, clus in enumerate(clusterers):
        temp = evaluations_df[[
            col for col in evaluations_df.columns if clus == col.split(param_delim, 1)[0]
        ]].transpose()

        temp.index = pd.MultiIndex.from_frame(pd.DataFrame(
            [{kv.split(val_delim)[0]: kv.split(val_delim)[1] for kv in i.split(param_delim, 1)[1:]}
             for i in temp.index]
        ).astype(float))
